    ...
    if (ITEM_TO_DATA_ACCESSORS.some((x) => parts[0].match(x))) {
        if (parts.length === 1) {
            ...
            return {
                ...
                replacementPrefix: `$('${startNodeName}').${accessorPrefix}`,
...
function resolveDuplicates(data, allNodeNames) {
    ...
    for (const mapping of data) {
        ...
        const hasKeyAndCollides = (key) => {
            ...
            if (!value)
                return false;
            return !(0, lodash_1.isEqual)(value, mapping);
        };
...
function applyExtractMappingToNode(node, parameterExtractMapping) {
    ...
    const applyMapping = (parameters, mapping) => {
        ...
        return (0, lodash_1.mapValues)(parameters, (v, k) => applyMapping(v, mapping[k]));
    };
...
function applyCanonicalMapping(mapping, getCanonicalData) {
    ...
    return (0, lodash_1.mapValues)(mapping, (v) => applyCanonicalMapping(v, getCanonicalData));
}
function extractReferencesInNodeExpressions(subGraph, nodeNames, insertedStartName, graphInputNodeNames) {
    ...
    const namesRegexp = '(' + nodeNames.map(lodash_1.escapeRegExp).join('|') + ')';
    ...
    const output = [];
    for (const node of subGraph) {
        const { result, usedMappings } = applyExtractMappingToNode((0, lodash_1.cloneDeep)(node), parameterTreeMappingByNode.get(node.name));
