...
let Push = class Push extends typed_emitter_1.TypedEmitter {
    ...
    handleRequest(req, res) {
        ...
        if (!pushRef) {
            ...
            const expectedHost = typeof headers['x-forwarded-host'] === 'string'
                ? headers['x-forwarded-host']
                : headers.host;
            if (expectedHost !== originHost) {
                this.logger.warn(`Origin header does NOT match the expected origin. (Origin: "${originHost}", Expected: "${expectedHost}")`, { headers: (0, lodash_1.pick)(headers, ['host', 'origin', 'x-forwarded-proto', 'x-forwarded-host']) });
                connectionError = 'Invalid origin!';
            }
        }
        if (connectionError) {
