...
let ActiveWorkflowManager = class ActiveWorkflowManager {
    constructor(logger, errorReporter, activeWorkflows, activeExecutions, externalHooks, nodeTypes, webhookService, workflowRepository, activationErrorsService, executionService, workflowStaticDataService, activeWorkflowsService, workflowExecutionService, instanceSettings, publisher, workflowsConfig) {
        this.logger = logger;
    ...
    async addActiveWorkflows(activationMode) {
        ...
        const batches = (0, lodash_1.chunk)(dbWorkflowIds, this.workflowsConfig.activationBatchSize);
    ...
    async activateWorkflow(workflowId, activationMode) {
        ...
        try {
            ...
            if (added.webhooks || added.triggersAndPollers) {
                this.logger.info(`   - ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)})`);
                this.logger.info('     => Started');
                this.logger.debug(`Activated workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)}`, {
            ...
            this.logger.info(`     => ERROR: Workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} could not be activated on first try, keep on trying if not an auth issue`);
