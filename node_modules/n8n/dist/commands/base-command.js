...
const modules_config_1 = require("../modules/modules.config");
...
class BaseCommand extends core_1.Command {
    constructor() {
        ...
        this.modulesConfig = di_1.Container.get(modules_config_1.ModulesConfig);
    ...
    async loadModules() {
        for (const moduleName of this.modulesConfig.modules) {
            let preInitModule;
            try {
                preInitModule = (await Promise.resolve(`${`../modules/${moduleName}/${moduleName}.pre-init`}`).then(s => __importStar(require(s))));
            }
            catch { }
            if (!preInitModule ||
                ...
                await Promise.resolve(`${`../modules/${moduleName}/${moduleName}.module`}`).then(s => __importStar(require(s)));
                this.modulesConfig.addLoadedModule(moduleName);
                this.logger.debug(`Loaded module "${moduleName}"`);
            }
        }
        await di_1.Container.get(module_registry_1.ModuleRegistry).initializeModules();
