...
const data_store_system_column_name_conflict_error_1 = require("./errors/data-store-system-column-name-conflict.error");
...
let DataStoreController = class DataStoreController {
    ...
    async addColumn(req, _res, dataStoreId, dto) {
        try {
            ...
            if (e instanceof data_store_not_found_error_1.DataStoreNotFoundError) {
                ...
                e instanceof data_store_system_column_name_conflict_error_1.DataStoreSystemColumnNameConflictError) {
                throw new conflict_error_1.ConflictError(e.message);
            }
            else if (e instanceof Error) {
                throw new internal_server_error_1.InternalServerError(e.message, e);
    ...
    async upsertDataStoreRow(req, _res, dataStoreId, dto) {
        try {
            ...
            if (dryRun) {
                return await this.dataStoreService.upsertRow(dataStoreId, req.params.projectId, dto, true, dryRun);
            }
            const returnData = dto.returnData;
            if (returnData) {
                return await this.dataStoreService.upsertRow(dataStoreId, req.params.projectId, dto, returnData, dryRun);
            }
            return await this.dataStoreService.upsertRow(dataStoreId, req.params.projectId, dto, returnData, dryRun);
        }
        catch (e) {
    ...
    async updateDataStoreRows(req, _res, dataStoreId, dto) {
        try {
            const dryRun = dto.dryRun;
            if (dryRun) {
                return await this.dataStoreService.updateRows(dataStoreId, req.params.projectId, dto, true, dryRun);
            }
            const returnData = dto.returnData;
            if (returnData) {
                return await this.dataStoreService.updateRows(dataStoreId, req.params.projectId, dto, returnData, dryRun);
            }
            return await this.dataStoreService.updateRows(dataStoreId, req.params.projectId, dto, returnData, dryRun);
        }
        catch (e) {
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], DataStoreController.prototype, "updateDataStoreRows", null);
__decorate([
