...
let DataStoreService = class DataStoreService {
    ...
    async insertRows(dataStoreId, projectId, rows, returnType = 'count') {
        ...
        const result = await this.dataStoreColumnRepository.manager.transaction(async (trx) => {
            ...
            return await this.dataStoreRowsRepository.insertRows(dataStoreId, rows, columns, returnType, trx);
        });
        this.dataStoreSizeValidator.reset();
        return result;
    }
    async upsertRow(dataTableId, projectId, dto, returnData = false, dryRun = false) {
        ...
        const result = await this.dataStoreColumnRepository.manager.transaction(async (trx) => {
            ...
            if (dryRun) {
                return await this.dataStoreRowsRepository.dryRunUpsertRow(dataTableId, dto.data, dto.filter, columns, trx);
            }
            const updated = await this.dataStoreRowsRepository.updateRows(dataTableId, dto.data, dto.filter, columns, true, trx);
            if (updated.length > 0) {
                return returnData ? updated : true;
            }
            const inserted = await this.dataStoreRowsRepository.insertRows(dataTableId, [dto.data], columns, returnData ? 'all' : 'id', trx);
    ...
    async updateRows(dataTableId, projectId, dto, returnData = false, dryRun = false) {
        ...
        const result = await this.dataStoreColumnRepository.manager.transaction(async (trx) => {
            ...
            this.validateUpdateParams(dto, columns);
            if (dryRun) {
                return await this.dataStoreRowsRepository.dryRunUpdateRows(dataTableId, dto.data, dto.filter, columns, trx);
            }
            return await this.dataStoreRowsRepository.updateRows(dataTableId, dto.data, dto.filter, columns, returnData, trx);
        });
        if (!dryRun) {
            this.dataStoreSizeValidator.reset();
        }
    ...
    async deleteRows(dataStoreId, projectId, dto, returnData = false, dryRun = false) {
        await this.validateDataStoreExists(dataStoreId, projectId);
        const result = await this.dataStoreColumnRepository.manager.transaction(async (trx) => {
            const columns = await this.dataStoreColumnRepository.getColumns(dataStoreId, trx);
            ...
            this.validateAndTransformFilters(dto.filter, columns);
            return await this.dataStoreRowsRepository.deleteRows(dataStoreId, columns, dto.filter, returnData, dryRun, trx);
        });
        if (!dryRun) {
            this.dataStoreSizeValidator.reset();
        }
